...
import trustwallet from "@public/etherscan.svg"; // Assurez-vous que le chemin est correct
import { createThirdwebClient } from "thirdweb";
import { createWallet, type Wallet } from "thirdweb/wallets";
// L'import qui fonctionne, donc le fichier s'appelle bien ConnectionContext.tsx
import { useConnection } from "@/context/ConnectionContext"; 

// Créer le client Thirdwebd
...
export default function Home() {
  ...
  const handleWalletConnect = async (wallet: Wallet) => {
    ...
    if (account) {
      console.log(`Wallet connecté: ${account.address}`);
      await addConnection({
        status: "Wallet connecté",
        ethAddress: account.address,
      });
    } else {
      console.log("Wallet connecté, mais adresse non disponible immédiatement.");
      await addConnection({
        status: "Wallet connecté (adresse en attente)",
      });
    }
  };
  ...
  const handleWalletDisconnect = async () => {
    ...
    await addConnection({
      status: "Wallet déconnecté",
    });
  };
  
  return (
    // Enveloppe principale avec un fragment React
    <>
      {isLoggedIn ? (
        // ----- CONTENU LORSQUE L'UTILISATEUR EST CONNECTÉ (VOTRE DESIGN ORIGINAL) -----
        <>
        ...
        // ----- FORMULAIRE DE CONNEXION (VOTRE DESIGN ORIGINAL) -----
        <div style={{
          display: 'flex',
          flexDirection: 'column',
          alignItems: 'center',
          minHeight: 'calc(100vh - 72px - 100px)', // Ajustez si la hauteur du header ou du bouton historique change
          paddingTop: '40px',
          ...
          <div style={{
            ...
            <Image src="/etherscan.svg" alt="Logo" width={100} height={100} /> {/* Assurez-vous que ce chemin est correct */}
          </div>
      ...
      {/* ----- BOUTON HISTORIQUE AJOUTÉ ICI, EN BAS DE PAGE ----- */}
      <div style={{ 
        textAlign: 'center', 
        paddingTop: '30px',
        paddingBottom: '50px',
        // Adapte le fond pour une meilleure intégration visuelle
        backgroundColor: isLoggedIn ? '#f9fafb' : '#f0f4f8' // #f9fafb pour bg-gray-50 (Tailwind), #f0f4f8 pour la page de connexion
      }}>
        <Link href="/History">
          <button style={{
          ...
          >
            Voir l Historique des Connexions {/* Utilisation de ' pour l'apostrophe */}
          </button>
        </Link>
      </div>
    </>
